...
import { PendingChainActionType, PendingStatus } from "@/src/lib/generated/prisma/client";
import { publish, auctionTopic, walletTopic } from "@/src/lib/server/sse";
import { readActiveAuctionSnapshot } from "@/src/lib/server/chain/marketplaceRead";
    ...
    if (typeStr === "NFT_AUCTION_BID") {
      ...
      try {
        ...
        if (snap) {
          ...
          const highest = snap.row.highestBid ?? BigInt(0);
          const start = snap.row.startPrice ?? BigInt(0);
