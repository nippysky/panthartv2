...
import { notifyDeployed } from "@/src/lib/telegram";
...
function validatePayload(p: DeployPayload) {
  ...
  if (!p.publicPriceWei || toWeiBigInt(p.publicPriceWei) <= BigInt(0))
  ...
  if (p.presale) {
    ...
    if (!p.presale.priceWei || toWeiBigInt(p.presale.priceWei) <= BigInt(0))
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (cfgAmountWei !== BigInt(0) && cfgAmountWei !== txValueWei) {
