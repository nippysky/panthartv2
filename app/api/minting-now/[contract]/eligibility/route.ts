...
import { normalizeAndDedupe, buildTree, getRootHex, getProofHex } from "@/src/lib/allowlist";
...
export async function GET(req: NextRequest, context: PageParams) {
  ...
  const includeProof = ["1", "true", "yes"].includes((url.searchParams.get("includeProof") || "").toLowerCase());
  ...
  try {
    ...
    const now = new Date();
    const presaleActive = !!col.presale && col.presale.startTime <= now && col.presale.endTime > now;
