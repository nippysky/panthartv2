...
export async function POST(req: NextRequest) {
  ...
  try {
      ...
      for (const log of receipt.logs) {
        try {
          ...
          const code = await client.getCode({
            ...
            blockNumber: BigInt(receipt.blockNumber) + BigInt(1), // code is available the block after deployment
          });
          if (code && code !== "0x") {
            contract = addr;
            break;
    ...
    async function readCommon1155() {
      ...
      const uri1 = await safeRead<string>(client, caddr, "uri", [BigInt(1)], ERC1155_SINGLE_ABI);
    ...
    async function readCommon721() {
      ...
      const tokenUri = await safeRead<string>(client, caddr, "tokenURI", [BigInt(1)], ERC721_DROP_ABI);
